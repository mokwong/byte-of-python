...
> 在 Windows 系统，默认情况下，是不支持 python3 这个命令的。在 Linux / macOS 可能有 python2，但是在 Windows 直接用 python 指向 Python 3。
...
## VS Code（或者衍生的 AI IDE）

以亚马逊的 kiro 为例：
1. 首先下载安装 kiro
2. 为 kiro 安装 python 插件
3. 切换 python 版本。kiro 默认会使用已安装的最新版本的 python，我要切换到 3.12.8。在 `view`->`Command Palette`->`Python: Select Interpreter`中，选择 `Python 3.12.8`即可
4. 右键或点击右上角的运行按钮，即可运行 python 代码
...
> python hello.py 和 py hello.py 有什么区别？
> py 是 Windows Python Launcher (py.exe)，它会根据注册表 + py.ini + 命令参数选择 Python 版本，然后运行制定版本的 python.exe，它可以用参数制定要使用的 python.exe 的版本。它只是一个启动器（Launcher），负责找到用户想用的 Python 版本，然后去调用对应的 python.exe。
...
如果你遭遇了什么错误，请确认是否已经\_正确地\_输入了上面所列出的内容，并尝试重新运行程序。要注意 **Python 是区分大小写**的，如 `print` 和 `Print` 是不同的——注意前者的 p 是小写的，而后者的 P 是大写的。此外，你需要确保每一行的第一个字符前面都没有任何空格或制表格——我们会在后面了解 [为什么这件事如此重要](07.basics.md#indentation)。
