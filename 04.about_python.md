...
> Python 是一款易于学习且功能强大的编程语言。 它具有高效率的数据结构，能够简单又有效地实现面向对象编程。Python 简洁的语法与动态输入之特性，加之其**解释性**语言的本质，使得它成为一种在多种领域与绝大多数平台都能进行脚本编写与应用快速开发工作的理想语言。
...
由于其开放源码的特性，Python 已被移植到其它诸多平台（意即它们已经过改动以保证其能正常工作）。如果你**小心地避开了所有系统依赖型的特性**。你所有的 Python 程序可以在其中任何一个平台上工作，不必作出任何改动。
...
> Python 和 Java 一样，都会先编译成字节码（.pyc），并在虚拟机上运行。但 Java 的 JVM 具有 JIT 编译能力，会将热点代码编译成机器码执行，而 Python（CPython）主要是解释执行字节码，因此性能通常低于 Java。
> CPython 是 Python 的官方参考实现，由 C 语言编写，负责将 Python 代码编译成字节码并在虚拟机中执行。虽然 Python 是一门语言规范，但 CPython 是最主流、默认的实现。除了 CPython 实现，还有其他的诸如 PyPy（支持 JIT 编译，性能更高）、Jython（运行在 Java 虚拟机上）、IronPython（运行在 .NET 上）等实现。
