...
> Java区分运行时异常和受检异常，我觉得是更好的。受检异常能让新手快速知道某行代码可能会抛出异常，从而避免了运行时异常。
> ai 给的结论： Java 的受检异常机制确实能帮助新手识别一部分可能异常的代码（如 IO 操作），但运行时异常的存在让这种保护不够完整。Python 虽然没有受检异常，但一致性更好，所有异常都靠开发者理解和测试来保证质量。